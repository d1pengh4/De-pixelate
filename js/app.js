...
async function setFile(f) {
  selectedFile = f; firstFrameData = null; videoW = 0; videoH = 0;
  selection = null; processBtn.disabled = true; selectionInfo.textContent = '';
  ...
  try {
    ...
    firstFrameData = imageData; videoW = W; videoH = H;
    ...
    selection = { x: 0, y: 0, w: 999999, h: 999999 };
...
function clearFile() {
  selectedFile = null; firstFrameData = null; selection = null;
  dragStart = null; isDragging = false; videoW = 0; videoH = 0;
  fileInput.value = '';
  fileInfo.classList.add('hidden'); dropZone.classList.remove('hidden');
  previewWrap.classList.add('hidden'); selectionInfo.textContent = '';
  ...
  return new Promise((resolve, reject) => {
    ...
    video.onloadedmetadata = () => {
      ...
      video.onseeked = () => {
        ...
        done(); resolve({ imageData: ctx.getImageData(0, 0, W, H), W, H });
      };
    };
    video.src = url;
  });
...
function drawSelection(sel) {
  ...
  ctx.restore();
  ctx.save(); ctx.strokeStyle = '#6c6fff';
  ctx.lineWidth = Math.max(1.5, cw / 400); ctx.setLineDash([6, 3]);
  ctx.strokeRect(x + 0.5, y + 0.5, w - 1, h - 1); ctx.restore();
  const hs = Math.max(6, Math.round(cw / 100)); ctx.save();
  ctx.fillStyle = '#6c6fff'; ctx.setLineDash([]);
...
previewCanvas.addEventListener('pointerdown', e => {
  if (!firstFrameData) return;
  e.preventDefault(); previewCanvas.setPointerCapture(e.pointerId);
  const { vx, vy } = toVideoCoords(e);
  dragStart = { vx, vy }; isDragging = true;
  selection = null; processBtn.disabled = true; selectionInfo.textContent = '';
  drawSelection(null);
});

previewCanvas.addEventListener('pointermove', e => {
  if (!isDragging || !dragStart) return; e.preventDefault();
  const { vx, vy } = toVideoCoords(e);
  const x = Math.min(dragStart.vx, vx), y = Math.min(dragStart.vy, vy);
  const w = Math.abs(vx - dragStart.vx), h = Math.abs(vy - dragStart.vy);
  drawSelection({ x: Math.round(x), y: Math.round(y), w: Math.round(w), h: Math.round(h) });
...
previewCanvas.addEventListener('pointerup', e => {
  ...
  if (w >= 10 && h >= 10) {
    ...
    selectionInfo.textContent = `선택 완료: ${selection.w} × ${selection.h}px  (${selection.x}, ${selection.y})`;
    processBtn.disabled = false;
  } else {
    selection = null; drawSelection(null);
    selectionInfo.textContent = '너무 작음 — 다시 드래그 (최소 10×10px)';
    processBtn.disabled = true;
  }
});

previewCanvas.addEventListener('pointercancel', () => { isDragging = false; dragStart = null; drawSelection(selection); });
...
async function startProcessing() {
  ...
  const sx = Math.max(0, selection.x), sy = Math.max(0, selection.y);
  const sw = Math.min(selection.w, (videoW || selection.w + sx) - sx);
  const sh = Math.min(selection.h, (videoH || selection.h + sy) - sy);
  const options = {
    windowPos:  [sy, sx],
    windowSize: [sh, sw],
  ...
  try {
    const result = await depixelate(selectedFile, options, (stage, pct) => setProgress(pct, stage));
    showResult(result);
  } catch (err) {
...
function showResult({ blob, reconData, cellH, cellW, frameCount }) {
  ...
  resultVideo.src = resultBlobUrl; resultVideo.load();
  downloadBtn.onclick = () => {
    const a = document.createElement('a'); a.href = resultBlobUrl;
    a.download = 'depixelated_' + (selectedFile?.name?.replace(/\.[^.]+$/, '') || 'result') + '.webm';
    a.click();
  };
  const tmpC = document.createElement('canvas');
  tmpC.width = reconData.width; tmpC.height = reconData.height;
  tmpC.getContext('2d').putImageData(reconData, 0, 0);
  tmpC.toBlob(imgBlob => {
    if (!imgBlob) return;
    resultImgUrl = URL.createObjectURL(imgBlob);
    imgDownBtn.onclick = () => {
      const a = document.createElement('a'); a.href = resultImgUrl;
      a.download = 'depixelated_' + (selectedFile?.name?.replace(/\.[^.]+$/, '') || 'result') + '.png';
      a.click();
    };
    imgDownBtn.classList.remove('hidden');
  }, 'image/png');
  const infoEl = $('result-info');
  if (infoEl) infoEl.textContent = `감지된 격자: ${cellH}×${cellW}px · 분석 프레임: ${frameCount}개`;
  showSection('result');
}

function showSection(name) {
  [uploadSec, progressSec, resultSec, errorSec].forEach(s => s.classList.add('hidden'));
  ({ upload: uploadSec, progress: progressSec, result: resultSec, error: errorSec }[name] || uploadSec).classList.remove('hidden');
}

function setProgress(pct, stage) {
  progressBar.style.width = Math.max(0, Math.min(100, pct)) + '%';
  stageEl.textContent = stage || ''; pctEl.textContent = Math.round(pct) + '%';
}
...
function reset() {
  if (resultBlobUrl) { URL.revokeObjectURL(resultBlobUrl); resultBlobUrl = null; }
  if (resultImgUrl)  { URL.revokeObjectURL(resultImgUrl);  resultImgUrl  = null; }
  resultVideo.src = ''; imgDownBtn.classList.add('hidden');
  clearFile(); setProgress(0, '대기 중...'); showSection('upload');
}
