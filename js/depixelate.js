...
async function extractFrames(file, onProgress, maxFrames = 300) {
  ...
  URL.revokeObjectURL(srcUrl); video.src = '';
...
function findPeriod(sig, minP = 4, maxP = 64) {
  ...
  let best = Math.max(minP, Math.min(16, maxP)), bestScore = -Infinity;
...
function detectCellSizeRobust(frames, wy, wx, wh, ww) {
  const nS = Math.min(frames.length, 9);
  const hArr = [], wArr = [];
  for (let i = 0; i < nS; i++) {
    const idx = Math.floor(i * frames.length / nS);
    const { cellH, cellW } = detectCellSize(frames[idx], wy, wx, wh, ww);
    hArr.push(cellH); wArr.push(cellW);
  }
  hArr.sort((a, b) => a - b); wArr.sort((a, b) => a - b);
...
async function fillGaps(accum, cnt, resH, resW, maxIters, onProgress) {
  ...
  for (let i = 0; i < N; i++) {
    if (cnt[i] > 0) {
      const n = cnt[i];
      image[i*4] = accum[i*4]/n; image[i*4+1] = accum[i*4+1]/n;
      image[i*4+2] = accum[i*4+2]/n; image[i*4+3] = 255; filled[i] = 1;
    }
  }
  const tmp = new Float32Array(N * 4), tmpFill = new Uint8Array(N);
  for (let iter = 0; iter < maxIters; iter++) {
    let anyNew = false; tmp.set(image); tmpFill.set(filled);
    for (let y = 0; y < resH; y++) {
      for (let x = 0; x < resW; x++) {
        const i = y * resW + x; if (filled[i]) continue;
        let r = 0, g = 0, b = 0, c = 0;
    ...
    if (!anyNew) break;
    if (iter % 5 === 0) {
      onProgress?.(`픽셀 복원 중... (${iter+1}/${maxIters})`, 80 + Math.min(10, Math.floor(10 * iter / maxIters)));
      await yieldToUI();
    }
  }
  return image;
...
async function createOutputVideo(file, reconData, wy, wx, W, H, duration, onProgress) {
  ...
  return new Promise((resolve, reject) => {
    ...
    recorder.onstop = () => { URL.revokeObjectURL(objUrl); video.src = ''; resolve(new Blob(chunks, { type: 'video/webm' })); };
    ...
    video.oncanplay = () => {
      ...
      const draw = () => {
        ...
        } else { cancelAnimationFrame(rafId); ctx.drawImage(video, 0, 0, W, H); ctx.putImageData(reconData, wx, wy); setTimeout(stopOnce, 400); }
      };
      video.onended = () => { cancelAnimationFrame(rafId); ctx.drawImage(video, 0, 0, W, H); ctx.putImageData(reconData, wx, wy); setTimeout(stopOnce, 400); };
...
async function depixelate(file, options, onProgress) {
  ...
  const { frames, W, H, duration } = await extractFrames(
    file, (cur, tot) => onProgress?.(`프레임 추출 중... (${cur}/${tot})`, 5 + Math.floor(25 * cur / tot)), maxFrames
  );
  ...
  if (wh < 8 || ww < 8) throw new Error('선택 영역이 너무 작습니다. 더 큰 영역을 드래그하세요.');
  let cellH, cellW;
  if (cellSize) { [cellH, cellW] = cellSize; }
  else { onProgress?.('격자 크기 감지 중...', 31); ({ cellH, cellW } = detectCellSizeRobust(frames, wy, wx, wh, ww)); }
  ...
  if (totalSamples === 0) throw new Error('픽셀을 누적할 수 없습니다. 셀 크기를 수동으로 지정해 보세요.');
  ...
  for (let i = 0; i < wh * ww * 4; i++) reconData.data[i] = Math.max(0, Math.min(255, Math.round(restored[i])));
  const blob = await createOutputVideo(file, reconData, wy, wx, W, H, duration, onProgress);
  onProgress?.('완료!', 100);
