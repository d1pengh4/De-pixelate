 ...
 *
 * Core idea: a moving mosaic window reveals different "real" pixels in each
 * frame. By sampling the centre of each mosaic cell across many frames and
 * accumulating the results, we reconstruct the original hidden content.
 */
...
function detectWindow(frame) {
  ...
  for (let y = yStart; y < yEnd; y++) {
    ...
    for (let x = 0; x < W; x++) {
      const i = (y * W + x) * 4;
      if ((data[i] + data[i+1] + data[i+2]) < 135) cnt++; // gray < 45
    }
  ...
  for (let x = 0; x < xEnd; x++) {
    ...
    for (let y = 0; y < H; y++) {
      const i = (y * W + x) * 4;
      if ((data[i] + data[i+1] + data[i+2]) < 135) cnt++;
    }
...
// ── 3. Mosaic cell-size detection ─────────────────────────────────────────────
function detectCellSize(frame, wy, wx, wh, ww) {
  const { data, width: W } = frame;

  // Row-difference signal (horizontal edges averaged across columns)
...
// ── 4. Mosaic grid offset detection ──────────────────────────────────────────
function findMosaicOffset(frame, wy, wx, wh, ww, cellH, cellW) {
  ...
  let mosaicY = 0;
  for (let y = margin; y < wh - margin - 1; y++) {
    if (Math.abs(hRow[y] - hRow[y + 1]) > 4) { mosaicY = y + 1; break; }
  }
  let mosaicX = 0;
  for (let x = margin; x < ww - margin - 1; x++) {
    if (Math.abs(vCol[x] - vCol[x + 1]) > 4) { mosaicX = x + 1; break; }
  }
...
// ── 5. Sample cell centres and accumulate ────────────────────────────────────
...
async function fillGaps(accum, cnt, resH, resW, maxIters, onProgress) {
  ...
  for (let iter = 0; iter < maxIters; iter++) {
    ...
    if (!anyNew) break;
    if (iter % 5 === 0) {
      onProgress?.(`이미지 복원 중... (${iter+1}/${maxIters})`, 84 + Math.floor(10 * iter / maxIters));
      await yieldToUI();
    }
  }
...
async function depixelate(file, options, onProgress) {
  ...
  // Step 2 – window region
  let wy, wx;
  if (windowPos) { [wy, wx] = windowPos; }
  else { const p = detectWindow(frames[0]); wy = p.wy; wx = p.wx; }
  ...
  else { const cs = detectCellSize(frames[0], wy, wx, wh, ww); cellH = cs.cellH; cellW = cs.cellW; }
  ...
  for (let i = 0; i < wh * ww * 4; i++) {
    imgData.data[i] = Math.max(0, Math.min(255, Math.round(image[i])));
  }
