...
async function extractFrames(file, onProgress, maxFrames = 300) {
  ...
  // Extract up to maxFrames, at least 15fps worth
...
// Autocorrelation-based cell size detection
...
// Score-based phase detection: find which offset aligns best with cell boundaries
function findMosaicOffset(frame, wy, wx, wh, ww, cellH, cellW) {
  ...
  // For each candidate boundary offset, sum diffs at that phase
  ...
  // Offset is the cell boundary position; center is half a cell beyond that
  return { mosaicY: bestOffY, mosaicX: bestOffX };
}

// Sample the center pixel of each mosaic cell and accumulate
...
// BFS gap-fill: spread sampled pixels to neighbours until all filled
...
// Compose output video: play original through canvas, overlay restored region, record WebM
